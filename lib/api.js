 ...
 * Resolves root-relative blog image paths to the API host.
 * Mirrors Laravel's normalizedImagePath() in app/Models/Post.php.
 */
export const getBlogImageUrl = (path) => {
  if (!path) return null;
  if (path.startsWith('http')) return path;
  // Replicate Laravel's normalization: prepend /uploads/ if missing
  if (!path.startsWith('/uploads/') && !path.startsWith('uploads/')) {
    path = '/uploads/' + path.replace(/^\/+/, '');
  }
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || `${getBaseHost()}/api`;
